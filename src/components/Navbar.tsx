...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white h-16 md:h-20 w-full relative" style={{boxShadow: '0 4px 12px rgba(0,0,0,0.15)'}}>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 border-t border-white/20 z-50">
          <div className="px-4 py-4 space-y-2">
            <a href="/" className="block px-4 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition-colors font-medium">
              Home
            </a>
            {(!user || user.publicMetadata?.role !== "consultancy") && (
              <a href="/categories" className="block px-4 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition-colors font-medium">
                Categories
              </a>
            )}
            <a href="/about" className="block px-4 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition-colors font-medium">
              About
            </a>
            <a href="/contact" className="block px-4 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition-colors font-medium">
              Contact
            </a>
            ...
            <div className="pt-2 border-t border-white/20">
              {!isClient || !isLoaded || !showButtons ? (
                ...
                <div style={{ opacity: buttonOpacity, transition: 'opacity 0.8s ease-out' }}>
                  <SignedOut>
                    <SignInButton mode="modal">
                      <button className="w-full bg-white text-indigo-600 px-4 py-3 rounded-lg font-medium hover:bg-indigo-50 transition-all shadow-sm">
                        Sign In
                      </button>
                    </SignInButton>
                  </SignedOut>
                  ...
                  <SignedIn>
                    <a
                      href={user?.publicMetadata?.role === "consultancy" ? "/consultancy-dashboard" : "/dashboard"}
                      className="block w-full bg-white text-indigo-600 px-4 py-3 rounded-lg font-medium hover:bg-indigo-50 transition-all shadow-sm text-center"
                    >
                      Dashboard
                    </a>
                  </SignedIn>
